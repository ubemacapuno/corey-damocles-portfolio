...
During build/deployment, I learned that to enable contact-form-submission with Netlify hosted sites, Netlify will search through the application's HTML for forms containing "data-netlify="true"" in order to add form functionality. However, when building the application with a front-end library such as SvelteKit, the form data will not be detected because when the application builds with Netlify, the HTML is dynamically rendered to JavaScript, meaning there is no longer any HTML for Netlify to search through. To get around this, the application needs to pre-render the HTML prior to the build so that Netlify can catch the proper data during the application build to set-up form submission functionality. For more info about this, check out this <a target="_blank" href="https://dev.to/swyx/how-to-use-sveltekit-with-netlify-forms-5gmj">blog post</a> I referenced to solve my issue.
